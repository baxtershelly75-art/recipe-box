...
const recipeDataFiles = Array.from({length:31},(_,i)=>`./data/recipes-${String(i+1).padStart(2,'0')}.json`);
...
function renderCook(){ const r=state.currentRecipe, step=r.cookSteps[state.cookIndex]; $('#cookRecipeTitle').textContent=r.title; $('#cookProgress').textContent=`${state.cookIndex+1} / ${r.cookSteps.length}`; $('#cookStep').textContent=scaledStepText(step,r); $('#cookBackBtn').disabled=state.cookIndex===0; $('#cookNextBtn').textContent=state.cookIndex===r.cookSteps.length-1?'DONE':'DONE / NEXT'; setupTimer(step.timerMinutes||0); }
...
function nextCook(){ clearTimer(); if(state.cookIndex<state.currentRecipe.cookSteps.length-1){state.cookIndex++;renderCook();}else{showView('#detailView');} }
